...
def _candidate_topics(question: str) -> list[str]:
    ...
    if parts and re.match(r"^(?:and|or)\s+", parts[-1], re.I):
        ...
        parts = [
            part.strip()
            for part in re.split(r"\s+(?:and|or)\s+", parts[0], flags=re.I)
        ]
    return parts
...
def compile_question(
    ...
    has_comparative = any(
        term in folded
        for term in ("least", "lowest", "fewest", "smallest", "youngest", "earliest", "minimum")
    )
    has_never = any(
        term in folded for term in ("never mentioned", "never raised", "never substantively")
    )
    if category == "absence" and not has_comparative and has_never:
        strategy = Strategy.ABSENCE_MATRIX
    elif category == "absence" and has_comparative:
