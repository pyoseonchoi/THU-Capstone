...
G8_STEPS = {s.strip() for s in os.environ.get("G8_STEPS", "1,2,3,4").split(",")
            if s.strip()}
...
def _claim_tokens(claim: dict) -> frozenset:
    ...
    tokens |= set(_SUPERLATIVE_MEANS.get(claim.get("superlative") or "", ()))
    ...
    for claim in claims:
        ...
        if scope_known:
            beaten = [r for r in beaten
                      if scopes.get(_entity_key(r.get("entity")), my_scope) == my_scope]
        if not beaten:
            continue
...
def adjudicate_conflicts(candidates: list[dict], question: str) -> list[dict]:
    ...
    try:
        ...
        return [c for c in shortlist if c["separated"]]
...
def answer_question(doc: dict, question: str, category: str | None) -> dict:
    ...
    if mode == "ledger":
        ...
        chunks = chunk_lines(ledger_text, WORDS_PER_READER)
    ...
    if mode == "ledger":
        ...
        verified, rejected = [], []
        for r in records:
            status = verify_record(r, against)
    ...
    try:
        if mode == "arc":
            ...
            final = call_llm(CROSS_SYNTHESIS_PROMPT.format(
                evidence=evidence_block, question=question,
                parts=parts_block)).strip()
        elif mode == "ledger":
