...
SUBMISSION = os.environ.get("SUBMISSION_FILE", "submission_g2.json")
...
def verify_record(record: dict, text: str) -> str:
    ...
    for m in re.finditer(re.escape(entity), text, re.I):
        ...
        if printed in line or bare in line.replace(",", ""):
            return "verified"

    return "entity_not_found" if not found_entity else "value_not_near_entity"
...
def _resolve_conflicts(numeric: list[dict]) -> tuple[list[dict], list[str]]:
    ...
    for key, group in by_entity.items():
        ...
        def strength(item):
            value, rs = item
            verified = sum(1 for r in rs if r.get("provenance") == "verified")
...
def tally(records: list[dict], question: str, source_text: str = "") -> dict | None:
    ...
    if source_text:
        ...
        for r in records:
            ...
            if verdict in ("verified", "unverifiable"):
                checked.append(r)
            else:
