...
# g13. Contradiction scored 0.0 while six other categories scored 26.7-100, and
# reading the run showed why: it was never an assembly problem.
#
#   d12  the pool held TWO candidates and the second was the answer. The
#        adjudicator model rejected both, the computed block went out empty,
#        and synthesis invented a pair from the reader lines.
#   d11  the pool held fifteen candidates and the answer was not among them.
#        The book says "the high plateau of Hardangervidda that dominates
#        NORWAY'S LARGEST NATIONAL PARK"; no reader emitted that claim. Both
#        records needed to refute it were present and verified.
#   d10  same shape. The book says "At 1085m, BRITAIN'S SECOND-HIGHEST PEAK";
#        readers emitted "the highest mountain in Wales" six times instead.
#
# So the missing half was always the CLAIM, never the figure.
#
#   textclaims  superlative and rank claims read out of the raw document
#               instead of out of whatever a reader volunteered
...
#   attrmatch   a pair is only a pair if both halves measure the same thing
#   route       candidates are matched to the question deterministically
#   onepair     one validated pair per question, with its comparison sentence
#               already written in Python -- never a pile
#
# ALL SIX ARE PURE PYTHON. Together they REMOVE a model call per contradiction
# question, because a Python-validated pair no longer goes to the adjudicator.
#
# Every one of them lives inside the ledger branch or in a function only the
# ledger branch calls. Nothing on the absence, needle, cross-section, arc or
# tally paths is touched -- proved byte-for-byte by ledger_isolation.py.
G13_FIXES = {f.strip() for f in os.environ.get(
    "G13_FIXES",
    "textclaims,namescope,rank,strictscope,attrmatch,route,onepair").split(",")
    if f.strip()}


def g13_on(name: str) -> bool:
    """Is this g13 fix live in the current run?"""
    return name in G13_FIXES or "all" in G13_FIXES


# g14. Parks scored 71.5% with aggregation at 35% and superlative at 47.2%, and
# reading the computed blocks showed the losses are not extraction failures.
#
#   answerfield  the computed block held matching_count 5 and records_returned
#                36, said only that it was "authoritative", and the model stated
#                36 -- the count of records SCANNED, not the count that matched.
#                Python now writes the answer sentence, as it does for
#                contradiction, and the model wraps prose around it.
#   ratefamily   'visitors per year' scores one hit in 'count' and one in 'age'
#                -- year -- so measurement_family calls it ambiguous and returns
#                None, while 'approximate number of visitors annually' resolves
#                to 'count'. Two labels for one measurement, two clusters, and
#                the unfamilied one becomes a greedy magnet that absorbed 'rock
#                eroded by glacier annually' and '22,000 pairs of seagulls' into
#                a ranking of visitor numbers. A rate's time word is its
#                denominator, not the thing being measured.
#   catroute     a question asking which COUNTRY entities are in is a group-by,
#                not a ranking. tally() only reaches categorical_tally() when no
#                record carries a number at all, so "how many parks are in
#                Croatia" ranked areas and answered 2 from one entity.
#   claimdomain  a superlative claim must be about the same measurement as the
#                figures offered to refute it. 'Norway's biggest single-drop
#                waterfall' was set against park AREAS because the claim names
#                no measurement the lexicon knows, so 'biggest' injected 'area'.
#   anchorstop   cross-section anchors came out as 'entries', 'happen', 'record'
#                -- the question's own phrasing, not the document's landmarks.
G14_FIXES = {f.strip() for f in os.environ.get(
    "G14_FIXES",
    "answerfield,ratefamily,magnitude,catroute,claimdomain,anchorstop,unitsane,"
    "classify,figlist"
    ).split(",")
    if f.strip()}


def g14_on(name: str) -> bool:
    """Is this g14 fix live in the current run?"""
    return name in G14_FIXES or "all" in G14_FIXES
...
CATEGORY_PROMPT = """Classify the question below into exactly ONE of these seven kinds.

aggregation      counting or totalling something across the WHOLE document.
                 "How many distinct external suppliers are named anywhere?"
superlative      the largest, earliest, most frequent -- a maximum or minimum
                 taken over the whole document.
                 "Which region recorded the single largest decline?"
absence          which of several named subjects the document NEVER
                 substantively discusses.
                 "Which of these four topics is never discussed?"
contradiction    two places where the document states things that cannot both
                 be true.
                 "Identify a place where the document contradicts itself."
cross_section    one fact combined with another stated far away, where the
                 document never makes the link itself.
                 "Which country is used both as an example of X and of Y?"
global_synthesis a theme and how it DEVELOPS across the whole document -- what
                 changes and what stays constant.
                 "How does the report's treatment of risk evolve?"
needle           one obscure fact stated in exactly one place.
                 "In what year was the first recorded eruption?"

QUESTION:
{question}

Reply with exactly one of: aggregation, superlative, absence, contradiction,
cross_section, global_synthesis, needle

One word, nothing else.
"""

_KNOWN_CATEGORIES = {"aggregation", "superlative", "absence", "contradiction",
                     "cross_section", "global_synthesis", "needle"}

# One call per distinct question, at most. question_mode is called more than
# once for some questions and a re-run must not re-spend.
_CLASSIFIED: dict[str, str | None] = {}
# What routed each question, so the log shows the path taken on the day.
ROUTE_SOURCE: dict[str, str] = {}


def _normalize_category(raw: str) -> str | None:
    """
    'Cross-Section', 'cross section', '  GLOBAL_SYNTHESIS  ' and 'The answer is:
    needle.' all resolve; 'banana' and a reply naming two categories do not.

    Two names in one reply is a REFUSAL, not a coin toss: the model was asked
    for one word and gave a sentence, and the wording heuristic behind this is
    a better bet than picking whichever appeared first.
    """
    text = re.sub(r"[^a-z]+", " ", (raw or "").lower()).strip()
    compact = re.sub(r"\s+", "_", text)
    found = [name for name in
             ("global_synthesis", "cross_section", "aggregation", "superlative",
              "absence", "contradiction", "needle")
             if name in compact]
    # 'cross_section' contains no other name, but a reply saying "not
    # aggregation -- superlative" contains two and is not an answer.
    return found[0] if len(found) == 1 else None


def classify_category(question: str) -> str | None:
    """
    Ask the small model which of the seven kinds a question is. Or return None.

    ONLY reached when the question file supplies no usable category. Measured on
    the practice set with categories stripped, the regex heuristic that fills
    that gap today misroutes TEN of twenty-one: every cross-section and every
    global-synthesis question falls through to prose, one absence question loses
    the zero-call Python path entirely, and a needle question is routed to the
    counting machinery. That is the risk this covers.

    A fallback for a fallback. Any failure -- broker error, unparseable reply, a
    word that is not one of the seven -- returns None and the regex heuristic
    runs exactly as before. It can never be a single point of failure, and it
    can never fire when a category was supplied.
    """
    if not g14_on("classify"):
        return None
    key = " ".join((question or "").split())
    if key in _CLASSIFIED:
        return _CLASSIFIED[key]
    try:
        reply = call_llm(CATEGORY_PROMPT.format(question=question),
                         model=CHEAP_MODEL)
    except Exception as e:                                    # noqa: BLE001
        _log(f"  ~ category classifier unavailable ({type(e).__name__}); "
             f"using the wording heuristic")
        _CLASSIFIED[key] = None
        return None
    name = _normalize_category(reply)
    if name is None:
        _log(f"  ~ category classifier returned {reply.strip()[:40]!r}, which is "
             f"not one of the seven; using the wording heuristic")
    _CLASSIFIED[key] = name
    return name
...
def question_mode(question: str, category: str | None) -> str:
    ...
    if category and category.lower().strip() in known:
        ROUTE_SOURCE[" ".join(question.split())] = "supplied category"
        return known[category.lower().strip()]

    # g14. The gap the regex heuristic fills today, filled better first. This is
    # unreachable when a category is supplied, so every practice document runs
    # the identical path it has always run.
    guessed = classify_category(question)
    if guessed and guessed in known:
        _log(f"  ~ no category supplied; classified as {guessed!r} by the "
             f"{CHEAP_MODEL} classifier")
        ROUTE_SOURCE[" ".join(question.split())] = f"classifier -> {guessed}"
        return known[guessed]
    ROUTE_SOURCE[" ".join(question.split())] = "wording heuristic"
...
# Words a QUESTION is built from. They occur in any document often enough to
# pass the rarity test and they locate nothing: one run's anchors came back as
# 'entries', 'events', 'record' and 'happen' -- the grammar of "Two park entries
# ... each record a climbing first ... when did they happen?" rather than
# landmarks in the book. This is English question vocabulary, not a fact about
# any document.
_QUESTION_FURNITURE = {
    "entries", "entry", "events", "event", "record", "records", "recorded",
    "happen", "happens", "happened", "describe", "described", "describes",
    "mention", "mentions", "mentioned", "state", "states", "stated", "give",
    "gives", "given", "name", "names", "named", "identify", "identifies",
    "explain", "explains", "actually", "formally", "several", "different",
    "following", "question", "answer", "example", "examples", "case", "cases",
    "appears", "appear", "appeared", "occurs", "occur", "shows", "show",
    "listed", "list", "lists", "consider", "considered", "together", "across",
    "pattern", "patterns", "reason", "reasons", "result", "results", "each",
    "both", "other", "others", "same", "which", "what", "where", "when",
    "connect", "connects", "connection", "relate", "related", "discussion",
}


def _is_question_furniture(phrase: str) -> bool:
    """Is every word of this phrase question grammar rather than subject matter?"""
    words = re.findall(r"[a-z']+", phrase.lower())
    if not words:
        return True
    return all(w in _QUESTION_FURNITURE or w in _STOPWORDS or w in _META
               for w in words)
...
def question_anchors(question: str, text: str) -> list[dict]:
    ...
    for phrase in candidates:
        if phrase in seen or len(phrase) < 5:
            continue
        if g14_on("anchorstop") and _is_question_furniture(phrase):
            continue
        seen.add(phrase)
...
# ------------------------------- THE PUBLISHED LIST (g14, priority 6)

IDENTIFIER_LIST_WINDOW = int(os.environ.get("IDENTIFIER_LIST_WINDOW", "40000"))
IDENTIFIER_LIST_MIN = int(os.environ.get("IDENTIFIER_LIST_MIN", "5"))


def _identifier_order(identifier: str) -> tuple:
    return tuple(int(p) if p.isdigit() else 0
                 for p in re.sub(r"[A-Z]", "", identifier).split("."))


def identifier_list_span(text: str, noun: str) -> set | None:
    """
    The document's own LIST of its numbered parts, if it printed one.

    document_identifiers() counts every mention anywhere, which answers "how
    many figures are REFERRED TO", not "how many figures are LISTED". On one
    report those differ by thirty-nine: 149 identifiers occur in the body and
    the published list holds 110 -- and the question says "Using the report's
    List of figures".

    A list has two properties prose does not, and both are checkable without
    knowing anything about the document:

      EACH ITEM APPEARS ONCE. Body text refers to a figure repeatedly; a list
      names it exactly once, so inside the list's span the distinct count and
      the mention count are nearly equal.
      THEY ASCEND. A list runs 1.1, 1.2, 2.1; prose jumps around.

    Requiring both over a sliding window finds the list where one exists and
    finds nothing where one does not -- and nothing is the safe answer, because
    the caller then counts the whole document exactly as before. Measured: it
    recovers 110 figures, 5 tables and 32 boxes on the report whose contents
    page is corrupted by dot-leader garbage, returns the identical count on a
    report whose list and body agree, and returns None for every noun on a
    report that prints no such list.
    """
    pattern = re.compile(rf"\b{re.escape(noun)}\s+"
                         rf"([A-Z]{{0,2}}\d*(?:\.\d+)+[a-z]?)\b", re.I)
    hits = [(m.start(), m.group(1).upper()) for m in pattern.finditer(text)]
    if len(hits) < IDENTIFIER_LIST_MIN:
        return None
    best = None
    for start in range(0, len(text), 2000):
        window = [(p, i) for p, i in hits
                  if start <= p < start + IDENTIFIER_LIST_WINDOW]
        if len(window) < IDENTIFIER_LIST_MIN:
            continue
        distinct = {i for _, i in window}
        if len(distinct) / len(window) < 0.9:
            continue                              # mentioned more than once
        order = [_identifier_order(i) for _, i in window]
        rising = sum(1 for a, b in zip(order, order[1:]) if b >= a)
        if rising / max(1, len(order) - 1) < 0.9:
            continue                              # not in order
        if best is None or len(distinct) > len(best):
            best = distinct
    return best
...
def settle_identifier_count(question: str, text: str) -> dict | None:
    ...
    from_list = {}
    if g14_on("figlist"):
        # Where the document printed its own list, count THAT. A figure the body
        # mentions five times is one figure, and a figure the body never
        # mentions is still listed. Nothing changes on a document that prints
        # no list, or whose list agrees with its body.
        for n in dict.fromkeys(nouns):
            span = identifier_list_span(text, n)
            if span and len(span) != len(available.get(n, ())):
                from_list[n] = span
                _log(f"  ~ {n}: the document's own list holds {len(span)}, "
                     f"against {len(available.get(n, ()))} mentioned anywhere; "
                     f"counting the list")
        available = {**available, **from_list}
    ...
    if from_list:
        answer += (" These totals are taken from the document's own printed "
                   "list, not from counting mentions in the body.")
    return {"answer": answer,
            "counted_from_the_documents_own_list": sorted(from_list) or None,
...
# Words that multiply the number in front of them. Not units: a unit describes
# WHAT is measured and a magnitude describes HOW MANY, and treating one as the
# other is how '15 million' came to rank as fifteen.
# SPELLED OUT ONLY. The abbreviations are ambiguous and one of them is lethal
# here: 'm' means million in a finance table and metres in this book, so
# admitting it turned a 4,102 m summit into 4.1 billion. 'k' and 'bn' carry the
# same risk on a document we have not seen. A magnitude that cannot be read
# without knowing the domain is not worth the elevation it destroys.
_MAGNITUDE = {"thousand": 1e3, "million": 1e6, "billion": 1e9, "trillion": 1e12}
...
def _parse_value(raw: str) -> tuple[float | None, str | None, str | None, float | None]:
    ...
    # A magnitude word is a multiplier, not a unit. '15 million' parsed as 15
    # and was ranked below a park reporting 800,000, so the most-visited park in
    # the book lost to one with a twentieth of its visitors. It also polluted
    # units_seen with 'million' and tripped the mixed-units warning. Any
    # document that writes large figures in words has this, and both documents
    # with score history do.
    if g14_on("magnitude") and tokens and tokens[0].strip(".") in _MAGNITUDE:
        value *= _MAGNITUDE[tokens.pop(0).strip(".")]
...
# "per year", "annually", "a day" -- the denominator of a rate. English marks
# rates the same way in any document, so this is grammar, not a fact about parks.
_RATE_PHRASE = re.compile(
    r"\b(?:per|a|each|every)\s+(?:year|annum|month|week|day|hour|season)\b"
    r"|\b(?:annually|yearly|monthly|weekly|daily|hourly|per\s*annum|p\.?a\.?)\b",
    re.I)
...
def measurement_family(attribute: str) -> str | None:
    ...
    """
    label = (attribute or "").lower()
    if g14_on("ratefamily"):
        # A RATE names its denominator, and the denominator is not what is being
        # measured. 'visitors per year' scored 'count' once (visitors) and 'age'
        # once (year), tied, and came back None -- so it clustered apart from
        # 'approximate number of visitors annually', which resolves cleanly to
        # 'count'. One measurement, two clusters, and the unfamilied one then
        # absorbed 'rock eroded by glacier annually' and '22,000 pairs of
        # seagulls' by token overlap and ranked them as visitor numbers.
        #
        # Stripping the rate phrase leaves 'visitors', which is the measurement.
        # 'year the park was established' carries no rate phrase and is left
        # alone, so the age family is untouched.
        label = _RATE_PHRASE.sub(" ", label)
    words = set(re.findall(r"[a-z]+", label)) - _ATTR_MODIFIER
    if not words:
        return None
    scores = {}
...
_CLAIM_BARE = {"national", "park", "parks", "reserve", "nature", "area",
               "areas", "region", "place", "site", "one", "country", "world",
               "europe", "european", "single", "drop", "kind", "type"}


def _claim_names_its_own_subject(claim: str) -> bool:
    """
    Does the claim name a concrete thing, or lean on its superlative?

    "Norway's largest national park" leans on 'largest', and the only way to
    check it is against areas. "Norway's biggest single-drop waterfall" names a
    waterfall, and no park's area can confirm or refute it.
    """
    # The scope is part of every claim and names no measurement, so strip it
    # first. Without this "Norway's largest national park" reads as naming the
    # subject 'norway' and stops implying 'area' -- which is the one thing it
    # must imply, since 'largest' is all it has to be checked against.
    text = re.sub(r"^\s*(?:the\s+)?[A-Z][\w'’-]+(?:\s+[A-Z][\w'’-]+)?['’]s\s+",
                  " ", claim or "")
    text = re.sub(r"\b(?:in|of|across|within|throughout)\s+(?:the\s+)?"
                  r"[A-Z][\w'’-]+(?:\s+[A-Z][\w'’-]+)?\b", " ", text)
    words = [w for w in re.findall(r"[a-z]+", text.lower())
             if w not in _CLAIM_BARE and w not in _ATTR_MODIFIER
             and w not in _STOPWORDS and w not in _SUPERLATIVE_MEANS
             and w not in _ORDINAL and len(w) > 3]
    return bool(words)
...
def _claim_tokens(claim: dict) -> frozenset:
    ...
    if g14_on("claimdomain") and _claim_names_its_own_subject(claim["claim"]):
        # "Norway's biggest single-drop WATERFALL" names what it is about. The
        # lexicon does not know 'waterfall', so measurement_family returns None,
        # the g10 guard below never fires, and 'biggest' injects 'area' -- which
        # matched park areas and set a waterfall claim against a 3,583 sq km
        # park in a question about park size. A claim carrying a concrete
        # subject noun implies no measurement of its own.
        implied = set()
    elif g10_on("claimfam") and measurement_family(claim["claim"]):
...
def find_unit_outliers(records: list[dict]) -> list[dict]:
    ...
    for r in records:
        if not r.get("unit"):
            continue
        if g14_on("unitsane") and not _UNIT_ONLY.match(r["unit"].strip()):
            # _parse_value falls back to "whatever token followed the number",
            # which is a unit only when the text was well formed. From a printed
            # range '370-550' it returns '-550', and from '17/29 sq km' it
            # returns '/29'; both then look like an entry printed in a unit of
            # its own. Two such artifacts tied with the real answer on a
            # question that asks WHICH park uses different units, and one of
            # them won the tie. find_label_unit_outliers has always applied this
            # test to the document's own labels -- the record side never did.
            continue
        tokens = _attr_tokens(r.get("attribute"))
        if not tokens:
            continue
...
def entity_scopes(records: list[dict]) -> dict[str, str]:
    ...
    if g13_on("namescope"):
        # An entity whose country no reader wrote down, scoped by its own name.
        #
        # This is not a new lexicon -- _SCOPE_ALIASES already says a park in
        # Svalbard is a park in Norway, and says so precisely because "a
        # comparison that ignores that reports a contradiction where none
        # exists". It was only ever consulted for scopes a reader had already
        # volunteered. Nordvest-Spitsbergen is the case it was written for and
        # the case it could not reach: no reader emitted its country, so the
        # one park that refutes "Norway's largest" ranked below two foreign
        # parks that refute nothing.
        for r in records:
            key = _entity_key(r.get("entity"))
            if not key or key in scopes:
                continue
            for name, canon in _SCOPE_CANON.items():
...
def check_claims(claims: list[dict], records: list[dict]) -> list[dict]:
    ...
    for claim in claims:
        ...
        # g13 reads the claim's own scope whether or not claimscope is on, since
        # strictscope needs it to decide what may refute the claim at all.
        claimed_scope = (claim_scope(claim["claim"])
                         if (g11_on("claimscope") or g13_on("strictscope")) else None)
        if my_scope is None:
        ...
        if g13_on("strictscope") and claimed_scope:
            # The claim names its own country, so a figure from an entity whose
            # country the document never states refutes nothing -- it is a guess
            # dressed as a finding. g11 only PREFERRED in-scope refuters and
            # kept the unscoped ones when there were none; that is how "the UK's
            # largest national park" came to be refuted by an Icelandic park,
            # and how d11 offered Cairngorms/Vatnajokull for a question that
            # says "another park IN THE SAME COUNTRY". A missing candidate
            # scores zero; a wrong one scores zero and can trip a forbidden
            # claim, so refusing is strictly better.
            beaten = [r for r in beaten
                      if scopes.get(_entity_key(r.get("entity"))) is not None
                      and _scope_compatible(
                          claimed_scope, scopes.get(_entity_key(r.get("entity"))))]
        if not beaten:
            continue
        ...
        out.append({
            ...
            "contradicted_by": [
                {"entity": r.get("entity"), "value": _display_value(r),
                 "where": r.get("where"),
                 # g14: the refuting record's own attribute, so the pair can be
                 # checked for measuring the same thing. This used to carry only
                 # 'where' -- a page number -- so attributes_agree compared a
                 # claim against a location string, found no family on either
                 # side, and passed everything through.
...
# ================================================= CLAIMS FROM THE TEXT (g13)
#
# find_label_unit_outliers already established the principle: "reading the raw
# text instead makes this immune to what any reader chose to report." Units
# needed it, and claims need it more, because a claim is a sentence rather than
# a number and a reader has far more latitude in choosing which one to copy.

_ORDINAL = {"first": 1, "second": 2, "third": 3, "fourth": 4, "fifth": 5,
            "sixth": 6, "seventh": 7, "eighth": 8, "ninth": 9, "tenth": 10}
_CARDINAL = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6,
             "seven": 7, "eight": 8, "nine": 9, "ten": 10}

_SUPER_WORD = (r"largest|biggest|highest|tallest|longest|deepest|oldest|"
               r"greatest|smallest|shortest|lowest|widest|most")

# The thing being ranked: a short lowercase noun phrase. Capped at four words
# because past that the match is running into the rest of the sentence.
_THING = r"[a-z][\w-]*(?:\s+[a-z][\w-]*){0,3}"

# Two shapes. Between them they cover every superlative sentence in the
# documents with score history, and they are English constructions rather than
# facts about any document.
#
#   A  possessive   "Norway's largest national park", "Britain's second-highest
#                   peak", "Austria's highest peak", "Europe's deepest canyon"
#   B  prepositional "the highest mountain in Wales", "the largest park in the Alps"
# NOT re.I on the whole pattern. Under re.IGNORECASE the [A-Z] in the scope
# group matches lowercase too, so "the high plateau of Hardangervidda that
# DOMINATES Norway's largest national park" captured the scope as "dominates
# Norway" and the claim came out reading "described as dominates Norway's
# largest national park". Case matters here -- a scope is a proper noun and a
# thing is not -- so only the closed word lists are case-folded, inline.
_CLAIM_A = re.compile(
    rf"\b(?P<scope>[A-Z][\w'’-]+(?:\s+[A-Z][\w'’-]+)?)['’]s\s+"
    rf"(?:(?P<ordinal>(?i:{'|'.join(_ORDINAL)}))[-\s]+)?"
    rf"(?P<super>(?i:{_SUPER_WORD}))\s+(?P<thing>{_THING})")

_CLAIM_B = re.compile(
    rf"\b(?i:the)\s+(?:(?P<ordinal>(?i:{'|'.join(_ORDINAL)}))[-\s]+)?"
    rf"(?P<super>(?i:{_SUPER_WORD}))\s+(?P<thing>{_THING}?)\s+"
    rf"(?i:in|of)\s+(?:(?i:the)\s+)?(?P<scope>[A-Z][\w'’-]+(?:\s+[A-Z][\w'’-]+)?)")

# "one of Europe's largest birds" asserts membership of a group, not a rank.
# Ten of the eleven matches in the parks book are this hedge; excluding it is
# what leaves the one real claim standing.
_CLAIM_HEDGE = re.compile(r"\b(?:one|some|many|few|any)\s+of\s+(?:the\s+)?$", re.I)

# Trailing words that are the sentence continuing, not part of the claim.
_CLAIM_TAIL = re.compile(
    r"\s+(?:has|have|had|is|are|was|were|will|would|can|could|may|might|does|"
    r"do|did|been|being|that|which|who|and|but|for|with|since|when|where|"
    r"sprouts?|sits?|stands?|lies?|rises?|runs?)\b.*$", re.I)


def _trim_claim(text: str) -> str:
    """'Norway's largest national park has for' -> \"Norway's largest national park\"."""
    return _CLAIM_TAIL.sub("", " ".join(text.split())).strip(" ,.;:")


TEXT_CLAIM_LIMIT = int(os.environ.get("TEXT_CLAIM_LIMIT", "400"))


def find_text_claims(text: str, names: list[str]) -> list[dict]:
    """
    Superlative and rank claims found in the document, attributed to an entity.

    The entity is the nearest preceding name the readers DID return, which is
    exactly how find_label_unit_outliers attributes its sites: the names still
    come from the records, only the claim comes from the text.

    A claim attached to the wrong entity would be worse than a missing one --
    it manufactures a conflict between two unrelated facts. Two things stop
    that. Attribution requires a name the readers actually reported, so a claim
    in a region with no reported entity is dropped rather than guessed at; and
    check_claims then requires the claimed entity to hold a figure for the
    measurement the claim is about, which discards the rest. Measured on the
    parks book, "Germany's second-largest river" attaches to a nearby hill and
    is then dropped because that hill has no river measurement.
    """
    out, seen = [], set()
    for pattern in (_CLAIM_A, _CLAIM_B):
        for m in pattern.finditer(text):
            if _CLAIM_HEDGE.search(text[max(0, m.start() - 24):m.start()]):
                continue
            scope = m.group("scope")
            if not scope or scope.lower() in _NOT_A_SCOPE:
                continue
            claim = _trim_claim(m.group(0))
            if len(claim) < 8:
                continue
            entity = _nearest_name(text, m.start(), names)
            if entity.startswith("("):
                continue
            key = (_entity_key(entity), claim.lower())
            if key in seen:
                continue
            seen.add(key)
            out.append({
                "entity": entity, "claim": claim, "source": "document text",
                "where": _where_of(text, m.start()),
                "from_text": True,
                "superlative": m.group("super").lower(),
                "claimed_rank": _ORDINAL.get(
                    (m.groupdict().get("ordinal") or "").lower()),
            })
            if len(out) >= TEXT_CLAIM_LIMIT:
                return out
    return out


def _sentence_subject(text: str, at: int, back: int = 500) -> str:
    """
    The capitalised subject of the sentence a claim sits in.

    _nearest_name can only attribute a claim to a name some reader happened to
    report FOR THIS QUESTION, and that is not the same set for every question.
    Measured: the sentence "The Cairngorms is the UK's largest national park
    ... that encompasses five of Britain's six highest summits" attributes to
    'Cairngorms National Park' when the pooled names are used and to 'Pen-y-Fan'
    -- 2,174 characters further away -- when only the records of the question
    about Snowdon are available, because no reader in that question had reason
    to emit a Cairngorms record.

    The sentence's own subject does not depend on any of that. Read the last
    sentence boundary before the claim and take the first capitalised run after
    it, which is where English puts the thing being described.
    """
    seg = text[max(0, at - back):at]
    cut = max(seg.rfind(". "), seg.rfind("! "), seg.rfind("? "), seg.rfind("\n"))
    sentence = seg[cut + 1:] if cut >= 0 else seg
    m = re.search(r"\b(?:The|A|An)?\s*([A-Z][\w'’-]{2,}(?:\s+[A-Z][\w'’-]+){0,3})",
                  sentence)
    if not m:
        return ""
    name = m.group(1).strip()
    if name.lower() in _NOT_A_SCOPE or len(name) < 4:
        return ""
    return name


def _where_of(text: str, at: int) -> str:
    """A page marker near a position, if the document carries any."""
    window = text[max(0, at - 4000):at]
    found = list(_PAGE_MARKER.finditer(window))
    if found:
        m = found[-1]
        return f"page {m.group(1) or m.group(2)}"
    return "stated in the document text"


# "five of Britain's six highest summits" -- a claim about an ORDERING, made by
# naming a subset and a total. The total is what makes it a ranking rather than
# the "one of Europe's largest" hedge, so it is required.
_GROUP_RANK = re.compile(
    rf"\b(?P<n>\d+|(?i:{'|'.join(_CARDINAL)}))\s+(?i:of)\s+(?:(?i:the)\s+)?"
    rf"(?P<scope>[A-Z][\w'’-]+(?:\s+[A-Z][\w'’-]+)?)['’]s\s+"
    rf"(?P<m>\d+|(?i:{'|'.join(_CARDINAL)}))\s+"
    rf"(?P<super>(?i:{_SUPER_WORD}))\s+(?P<thing>[a-z][\w-]*)")
...
def _as_int(word: str) -> int | None:
    ...
    if word.isdigit():
        return int(word)
    return _CARDINAL.get(word)


def find_group_rank_claims(text: str, names: list[str]) -> list[dict]:
    """
    Claims of the form "N of SCOPE's M highest THINGs", with the place they
    belong to.

    This is the shape the value ledger structurally cannot see. It states no
    figure, so nothing disagrees with anything; what it fixes is an ORDERING,
    and an ordering is refuted by a rank claim rather than by a number.

    Requiring the total ('six') is the whole discriminator. Without it the
    pattern also matches "one of Europe's largest birds" -- ten times in the
    parks book against one real hit.
    """
    out = []
    for m in _GROUP_RANK.finditer(text):
        n, total = _as_int(m.group("n")), _as_int(m.group("m"))
        if not n or not total or n > total:
            continue
        # The sentence's own subject first; a reported name only as a fallback.
        holder = _sentence_subject(text, m.start()) or _nearest_name(text, m.start(), names)
        if not holder or holder.startswith("("):
            continue
        out.append({"holder": holder, "subset": n, "total": total,
                    "at": m.start(),
                    "scope": _SCOPE_CANON.get(m.group("scope").lower(),
                                              m.group("scope").lower()),
                    "superlative": m.group("super").lower(),
...
def _distance_to(text: str, entity: str, at: int) -> int:
    """How far the nearest mention of an entity sits from a position."""
    best = 10 ** 9
    for m in re.finditer(re.escape(entity), text, re.I):
        best = min(best, abs(m.start() - at))
    return best


def find_rank_conflicts(claims: list[dict], group_claims: list[dict],
                        records: list[dict], text: str = "") -> list[dict]:
    """
    A claimed rank against the ordering the document states elsewhere.

    Two independent tests; either one is enough.

      GROUP. The document says N of a scope's M best THINGs belong somewhere
      else. An entity outside that group claiming rank R <= N cannot be right:
      there are already N ahead of it. This is what d10 needs. Snowdon is
      called "Britain's second-highest peak"; the Cairngorms entry says the
      park "encompasses five of Britain's six highest summits"; Snowdon is not
      in the Cairngorms, so rank 2 is impossible.

      VALUE. Count the entities in the SAME KNOWN SCOPE with a better figure
      for the same measurement. If that count is at least R, rank R is
      impossible. Scope must be KNOWN on both sides here -- unknown is treated
      as compatible everywhere else in this file, and that latitude is right
      when the question is "does anything refute this at all", but for counting
      how many entities are ahead it would silently compare a Welsh mountain
      against Spanish and Italian ones. Measured: eighteen peaks in the parks
      book are taller than Snowdon and every one of them is abroad.
    """
    scopes = entity_scopes(records)
    out = []
    for claim in claims:
        rank = claim.get("claimed_rank")
        if not rank:
            continue
        subject = _entity_key(claim["entity"])
        scope = claim_scope(claim["claim"]) or scopes.get(subject)
        if not scope:
            continue
        ...
        mine = [r for r in records
                if r.get("normalized") is not None
                and _entity_key(r.get("entity")) == subject
                and _claim_matches(wanted, _attr_tokens(r.get("attribute")))]
        my_value = max((r["normalized"] for r in mine), default=None)

        # GROUP test.
        for g in group_claims:
            if not _scope_compatible(scope, g["scope"]):
                continue
            if _entity_key(g["holder"]) == subject:
                continue                      # the claim's own home
            if rank > g["subset"]:
                continue                      # consistent: it can sit below them
            # Family equality, not token overlap, for the figures offered as
            # evidence. Overlap lets 'peak annual population' answer a question
            # about a summit -- the trap that once reported a 135,000 m peak --
            # because both labels contain the token 'peak'. measurement_family
            # scores that label in two families at once and returns None, so an
            # equality test drops it and a token test does not.
            family = measurement_family(claim["claim"])
            inside = [r for r in records
                      if r.get("normalized") is not None
                      and _entity_key(r.get("entity")) != subject
                      and (measurement_family(r.get("attribute")) == family
                           if family else
                           _claim_matches(wanted, _attr_tokens(r.get("attribute"))))
                      and _scope_compatible(
                          scope, scopes.get(_entity_key(r.get("entity"))))
                      and (my_value is None or r["normalized"] > my_value)]
            # By where they are printed, not by how big they are. A group
            # claim is made inside one entry and the figures that bear it out
            # are printed in that same entry; sorting by magnitude instead
            # reaches across the whole book and answers a question about a
            # Welsh mountain with a Spanish one. Measured: sorting by size
            # offered Mulhacen (3,479 m, 139,000 characters away) where the
            # entry's own Ben Macdui (1,309 m, 1,800 away) is the figure the
            # claim is actually about.
            if text:
                inside.sort(key=lambda r: _distance_to(text, r.get("entity") or "",
                                                       g.get("at", 0)))
            else:
                inside.sort(key=lambda r: -r["normalized"])
            out.append({
                "kind": "rank_conflict", "test": "group",
                "entity": claim["entity"], "claim": claim["claim"],
                "claim_where": claim.get("where"),
                "claimed_rank": rank, "scope": scope,
                "claimed_value": (_display_value(mine[0]) if mine else None),
                "group_claim": g["claim"], "group_holder": g["holder"],
                "group_where": g.get("where"),
                "group_subset": g["subset"], "group_total": g["total"],
                "contradicted_by": [
                    {"entity": r.get("entity"), "value": _display_value(r),
                     "where": r.get("where"),
                     "attribute": r.get("attribute")} for r in inside[:3]],
            })
            break
        ...
        if my_value is None:
            continue
        ahead = [r for r in records
                 if r.get("normalized") is not None
                 and _entity_key(r.get("entity")) != subject
                 and _claim_matches(wanted, _attr_tokens(r.get("attribute")))
                 and scopes.get(_entity_key(r.get("entity"))) is not None
                 and _scope_compatible(scope, scopes.get(_entity_key(r.get("entity"))))
                 and r["normalized"] > my_value]
        seen_keys, distinct = set(), []
        for r in sorted(ahead, key=lambda r: -r["normalized"]):
            k = _entity_key(r.get("entity"))
            if k in seen_keys:
                continue
            seen_keys.add(k)
            distinct.append(r)
        if len(distinct) >= rank:
            out.append({
                "kind": "rank_conflict", "test": "value",
                "entity": claim["entity"], "claim": claim["claim"],
                "claim_where": claim.get("where"),
                "claimed_rank": rank, "scope": scope,
                "claimed_value": _display_value(mine[0]),
                "computed_rank": len(distinct) + 1,
                "contradicted_by": [
                    {"entity": r.get("entity"), "value": _display_value(r),
                     "where": r.get("where"),
                     "attribute": r.get("attribute")} for r in distinct[:3]],
            })
    return out
...
# ============================================ CANDIDATE ASSEMBLY (g13)
#
# What the last run did with its candidates: handed the whole list to one model
# call, kept whatever came back, and let the synthesis model pair the survivors
# however it liked. The three answers that came out of that had the right facts
# in them and the wrong pairings between them -- a highest-peak claim set
# against an area figure, and a comparative sentence ("lower than Scafell Pike
# at 978 meters") whose inequality is false in the direction it was written.
#
# Below, Python picks ONE candidate, checks the comparison itself, and writes
# the sentence. The model receives a finished claim and wraps prose around it.


def _candidate_attributes(c: dict) -> tuple[str | None, str | None]:
    """The two things a candidate compares, as measurement families."""
    if c["kind"] == "value_conflict":
        parts = (c.get("attribute") or "").split(" / ")
        return (measurement_family(parts[0]),
                measurement_family(parts[-1]))
    if c["kind"] == "unit_outlier":
        f = measurement_family(c.get("attribute"))
        return f, f
    # claim_vs_data and rank_conflict: the claim on one side, the figures that
    # beat it on the other.
    beaten = c.get("contradicted_by") or []
    return (measurement_family(c.get("claim")),
            measurement_family(beaten[0].get("attribute")) if beaten else None)


def attributes_agree(c: dict) -> bool:
    """
    Both halves of a pair must measure the same thing.

    d11's answer set "Jotunheimen ... called Norway's highest peak
    (Galdhopiggen)" against "Hardangervidda ... an area of 3430 sq km". Those
    are a height and an area; nothing about them can conflict. Where a family
    cannot be determined for one side the pair is KEPT -- measurement_family
    returns None for any label outside its lexicon, and rejecting on None would
    throw away every candidate on a document whose vocabulary it does not
    cover.
    """
    a, b = _candidate_attributes(c)
    if a is None or b is None:
        return True
    return a == b


def _candidate_terms(c: dict) -> tuple[set, set]:
    """(entity words, measurement words) a candidate is about."""
    entities = {c.get("entity") or "", c.get("group_holder") or ""}
    entities |= {e.get("entity") or ""
                 for e in (c.get("contradicted_by") or c.get("entities") or [])}
    ent_words = {w for name in entities
                 for w in re.findall(r"[a-z]{4,}", (name or "").lower())}
    attr = " ".join(str(c.get(k) or "") for k in
                    ("attribute", "claim", "odd_unit", "majority_unit", "thing"))
    # What KIND of finding this is, in the words a question would use to ask
    # for it. d12 asks "reported in different UNITS" and the unit outlier it
    # wants shares only the word 'area' with it, while a superlative claim
    # about a different park shares four -- so the wrong candidate outranked
    # the right one purely on incidental vocabulary.
    kind_words = {
        "unit_outlier": "unit units different others reported measure",
        "rank_conflict": "rank ranked ranking order highest second among",
        "claim_vs_data": "calls claim described largest another same",
        "value_conflict": "value values figure figures different twice",
    }.get(c["kind"], "")
    words = re.findall(r"[a-z]{4,}", (attr + " " + kind_words).lower())
    return ent_words, {w for w in words if w not in _STOPWORDS}


def route_score(c: dict, question: str) -> int:
    """
    How well a candidate answers THIS question, by string overlap with the
    question's own words.

    Deterministic string matching against the QUESTION, not similarity search
    over the document: no chunk is ranked or filtered, and every slice was
    still read. It exists because the unit outlier that d12 asks for was also
    offered to d10 and d11, which ask for something else.

    An entity name is worth more than a measurement word. "Snowdon" appearing
    in the question is near-conclusive; "area" is shared by half the pool.
    """
    words = {w for w in re.findall(r"[a-z]{4,}", question.lower())
             if w not in _STOPWORDS}
    ent_words, attr_words = _candidate_terms(c)
    return 3 * len(words & ent_words) + len(words & attr_words)


def _bigger_word(smaller: bool) -> str:
    return "smaller" if smaller else "larger"


def comparison_sentence(c: dict) -> str | None:
    """
    The finished sentence, with its arithmetic already checked.

    Returns None when the comparison does not hold, and that is the point: a
    candidate whose own numbers do not support the sentence never reaches the
    model. The last run stated that 1085 m is "lower than ... Scafell Pike at
    978 meters"; no step anywhere had checked the inequality.
    """
    if c["kind"] == "value_conflict":
        ...
        if len(vals) < 2:
            return None
        a, b = vals[0], vals[1]
        return (f"{c['entity']} is given a {c['attribute']} of {a['value']} "
                f"in {a.get('where') or 'one place'} and of {b['value']} in "
                f"{b.get('where') or 'another'}.")

    if c["kind"] == "unit_outlier":
        odd = (c.get("entities") or [{}])[0]
        if not odd.get("entity"):
            return None
        return (f"{odd['entity']} reports its {c.get('attribute')} as "
                f"{odd.get('value')} {c.get('odd_unit')}, while "
                f"{c.get('majority_count')} other entries give the same "
                f"measurement in {c.get('majority_unit')}.")
    ...
    if c["kind"] == "rank_conflict" and c.get("test") == "group":
        value = f" at {c['claimed_value']}" if c.get("claimed_value") else ""
        where = f" ({c['group_where']})" if c.get("group_where") else ""
        sentence = (f"{c['entity']} is described as {c['claim']}{value}, but "
                    f"the document also states that {c['group_claim']} are in "
                    f"{c['group_holder']}{where} -- so at least "
                    f"{c['group_subset']} rank above it and it cannot be "
                    f"number {c['claimed_rank']}.")
        if other.get("value"):
            sentence += (f" {other['entity']} is given as {other['value']}"
                         + (f" ({other['where']})" if other.get("where") else "")
                         + ".")
        return sentence

    # claim_vs_data, and the value form of rank_conflict: both assert that a
    # figure beats the claimed one. VERIFY IT.
    mine = _parse_value(str(c.get("claimed_value") or ""))[3]
    theirs = _parse_value(str(other.get("value") or ""))[3]
    if mine is None or theirs is None:
        return None
    smaller = (c.get("superlative") or "") in {"smallest", "shortest", "lowest"}
    if (theirs >= mine) if smaller else (theirs <= mine):
        return None                       # the inequality does not hold: drop it
    rank = (f", but it is only number {c['computed_rank']} by the document's own "
            f"figures" if c.get("computed_rank") else "")
    return (f"{c['entity']} is described as {c['claim']} at "
            f"{c['claimed_value']}"
            + (f" ({c['claim_where']})" if c.get("claim_where") else "")
            + f", but {other['entity']} is given "
              f"{other['value']}"
            + (f" ({other['where']})" if other.get("where") else "")
            + f", which is {_bigger_word(smaller)}{rank}.")


def select_one_conflict(candidates: list[dict], question: str) -> dict | None:
    """
    Exactly one candidate, fully validated, with its sentence written.

    Ranked by how well the candidate answers THIS question, then by whether the
    two halves came from different parts of the document -- a disagreement
    inside one slice is usually two columns of one table misread. A candidate
    whose sentence cannot be built, or whose arithmetic does not hold, is
    skipped rather than repaired.
    """
    scored = []
    for c in candidates:
        if g13_on("attrmatch") and not attributes_agree(c):
            continue
        sentence = comparison_sentence(c)
        if not sentence:
            continue
        # ONE pair means one. The sentence uses the first refuting figure, so
        # the rest are trimmed rather than shipped alongside it -- a second
        # value in the block is a second thing for the model to pair with, and
        # pairing freely is the whole failure this replaces. Nordvest-
        # Spitsbergen's marine area (3,500) sat next to its land area (3,583)
        # in exactly the question that turns on which of them is meant.
        trimmed = {**c, "comparison_sentence": sentence}
        if trimmed.get("contradicted_by"):
            trimmed["contradicted_by"] = trimmed["contradicted_by"][:1]
        if trimmed.get("values"):
            trimmed["values"] = trimmed["values"][:2]
        if trimmed.get("entities"):
            trimmed["entities"] = trimmed["entities"][:1]
        scored.append((route_score(c, question) if g13_on("route") else 0,
                       1 if c.get("separated", True) else 0,
                       1 if c["kind"] == "rank_conflict" else 0,
                       trimmed))
    if not scored:
        return None
    scored.sort(key=lambda t: (t[0], t[1], t[2]), reverse=True)
    if g13_on("route") and scored[0][0] == 0:
        # Nothing in the pool has a word in common with the question. Saying so
        # beats offering the best of a list that is about something else.
        return None
    return scored[0][3]
...
# ------------------------------------- CATEGORICAL ROUTING (g14, priority 3)

def categorical_records(records: list[dict]) -> list[dict]:
    """Records whose value is a CATEGORY name rather than a figure."""
    return [r for r in records
            if r.get("normalized") is None
            and (r.get("raw_value") or "").strip()
            and len((r.get("raw_value") or "").strip()) < 60]


def question_groups(question: str, records: list[dict]) -> list[str]:
    """
    Category values the question names that the records actually use.

    This is what tells a group-by from a ranking, and it is decided by the data
    rather than by wording. "How many of the parks are in Croatia and how many
    are in Montenegro?" names two values that appear verbatim as the value of a
    'country' record; "how many parks report a highest point of 3000 metres or
    more?" names none. No list of countries is hardcoded anywhere -- the
    document supplies the categories and the question is matched against them.

    Without this, tally() reaches categorical_tally() only when NOT ONE record
    carries a number, which for a question about countries is never true: the
    same readers also report areas. So the Croatia question ranked areas,
    resolved to a single entity, and answered "2 parks are in Croatia" from a
    distinct_entities of 1.
    """
    values = {(r.get("raw_value") or "").strip().lower()
              for r in categorical_records(records)}
    values = {v for v in values if v and not _NUMBER.search(v)}
    if not values:
        return []
    named = []
    for m in re.finditer(r"\b([A-Z][\w'’-]+(?:\s+[A-Z][\w'’-]+)?)", question):
        ...
        for v in values:
            # The record may say "Wales, UK" where the question says "Wales".
            if phrase == v or phrase in v.split(", "):
                if phrase not in named:
                    named.append(phrase)
    return named


def grouped_tally(records: list[dict], question: str,
                  groups: list[str]) -> dict | None:
    """
    Count the distinct entities in each category the question named.

    One entity is counted once per category, folded on _entity_key so that
    'Ordesa' and 'Ordesa National Park' are one park, and only categories the
    question actually asked about are reported.
    """
    members: dict[str, set] = {g: set() for g in groups}
    names: dict[str, dict] = {g: {} for g in groups}
    for r in categorical_records(records):
        value = (r.get("raw_value") or "").strip().lower()
        key = _entity_key(r.get("entity"))
        if not key:
            continue
        for g in groups:
            if value == g or g in [p.strip() for p in value.split(",")]:
                members[g].add(key)
                names[g].setdefault(key, (r.get("entity") or "").strip())
    if not any(members.values()):
        return None
    return {
        "counted_by": "the category each entity was reported under",
        "groups": {g: {"count": len(members[g]),
                       "entities": sorted(names[g].values())} for g in groups},
        "note": ("Each entity was assigned to the category the readers reported "
                 "for it and counted once, however many slices mentioned it. "
                 "Only the categories the question names are reported."),
    }
...
def tally(records: list[dict], question: str, source_text: str = "",
    ...
    # g14 priority 3. A question naming a category the records use is a
    # group-by, and must not be answered by ranking numbers that happen to be
    # in the same pool.
    if g14_on("catroute"):
        groups = question_groups(question, records)
        if groups:
            grouped = grouped_tally(records, question, groups)
            if grouped:
                if rejected:
                    grouped["rejected_unverifiable"] = {
                        k: v[:12] for k, v in rejected.items()}
                _log(f"  ~ categorical question: counting entities per "
                     f"{', '.join(groups)} rather than ranking figures")
                return (with_answer_sentence(grouped, question)
                        if g14_on("answerfield") else grouped)
    ...
    if g14_on("answerfield"):
        summary = with_answer_sentence(summary, question)
    return summary


# ------------------------------------- THE ANSWER SENTENCE (g14, priority 1)
#
# The computed block was told it was authoritative and never told WHICH of its
# numbers was the answer. For one counting question it carried
#
#     "records_returned": 36,      how many record lines the readers emitted
#     "distinct_entities": 19,     how many entities those lines named
#     "matching_count": 5,         how many met the question's threshold
#
# and the answer stated 36. The model was not disobeying; it was choosing, and
# nothing had told it that only one of the three answers the question.
#
# The contradiction path already solved this, and it took that category from
# 0.0 to 66.7 in one generation: Python writes the sentence, the model wraps
# prose around a claim whose arithmetic is already fixed. Same move here.

_SUPERLATIVE_ASK = re.compile(
    r"\b(largest|biggest|greatest|highest|tallest|longest|deepest|oldest|most|"
    r"smallest|shortest|lowest|fewest|widest|youngest|earliest)\b", re.I)
_COUNT_ASK = re.compile(r"\bhow many\b|\bnumber of\b|\bhow much\b|\bcount\b", re.I)


def _listed(entities: list[str], limit: int = 40) -> str:
    kept = entities[:limit]
    tail = "" if len(entities) <= limit else f", and {len(entities) - limit} more"
    return "; ".join(kept) + tail


def with_answer_sentence(summary: dict, question: str) -> dict:
    """
    Name the one number that answers the question, and write the sentence.

    Which number depends on what was asked, and the question says so plainly:
    a stated threshold makes it the count that met the threshold; a superlative
    makes it the extreme; a bare 'how many' makes it the number of distinct
    entities. Nothing here reads the document -- it reads the question and the
    arithmetic Python already did.
    """
    if summary.get("groups"):
        parts = [f"{g['count']} in {name.title()}"
                 + (f" ({'; '.join(g['entities'])})" if g["entities"] else "")
                 for name, g in summary["groups"].items()]
        first = next(iter(summary["groups"].values()))
        return {
            "ANSWER_VALUE": first["count"],
            "ANSWER_SENTENCE": "There are " + ", and ".join(parts) + ".",
            "answer_taken_from": "groups[].count",
            "how_to_use": (
                "Each category's count and the entities in it are given under "
                "'groups'. State every category the question asked about, with "
                "its count AND the names. No other number in this block answers "
                "the question."),
            **summary,
        }

    threshold = summary.get("threshold")
    maximum, minimum = summary.get("maximum"), summary.get("minimum")
    wants_min = bool(re.search(r"\b(smallest|shortest|lowest|fewest|youngest|"
                               r"least|earliest)\b", question, re.I))

    if threshold:
        value = summary.get("matching_count")
        names = summary.get("matching_entities") or []
        sentence = (f"{value} of them meet the stated condition "
                    f"({threshold['operator']} {threshold['value']:g}). "
                    f"They are: {_listed(names)}.")
        field = "matching_count"
    elif _SUPERLATIVE_ASK.search(question) and (minimum if wants_min else maximum):
        pick = minimum if wants_min else maximum
        value = pick["value"]
        sentence = (f"{pick['entity']} is the answer, with {pick['value']}"
                    + (f" ({pick['where']})" if pick.get("where") else "") + ".")
        field = "minimum" if wants_min else "maximum"
    elif _COUNT_ASK.search(question):
        value = summary.get("distinct_entities")
        names = summary.get("matching_entities") or []
        sentence = (f"{value} distinct entities were found. "
                    f"They are: {_listed(names)}.")
        field = "distinct_entities"
    else:
        return summary
    ...
    return {
        ...
        "answer_taken_from": field,
        "how_to_use": (
            "ANSWER_VALUE is the number that answers the question. It was "
            f"computed as '{field}'. Every other number in this block describes "
            "how the calculation was done -- how many record lines the readers "
            "emitted, how many entities they named, what was set aside -- and "
            "NONE of them answers the question. State ANSWER_VALUE and no other "
            "count."),
        **summary,
    }
...
# g13. The model receives ONE finished comparison and wraps prose around it.
# The previous prompt forbade inventing a pair and the model invented one
# anyway, three times out of three -- so the guard cannot live in the wording.
# It lives upstream: there is now nothing else in the block to pair.
ONEPAIR_SYNTHESIS_PROMPT = """Write one final answer to the question below.

Python has already found the contradiction, checked it against the raw text and
verified its arithmetic. THE FINDING IS SETTLED. Your job is to state it in full
sentences and add the surrounding detail the readers supply -- not to look for a
contradiction, not to weigh this one, and not to find a better one.

========================= THE VERIFIED CONTRADICTION =========================
{finding}
==============================================================================

============================== HOW TO ANSWER ==============================

1. OPEN WITH THE SENTENCE ABOVE, in your own words but with EVERY figure,
   entity name, unit and location exactly as given.
2. Then say plainly what the two statements are and where each is stated.
3. Then add any detail from the reader lines that bears on THIS contradiction --
   the wording the document uses, the section it sits in, a figure that makes
   the conflict clearer. Length costs nothing.

============================== HARD RULES ==============================

- YOU MAY NOT STATE ANY NUMERIC COMPARISON THAT IS NOT IN THE BLOCK ABOVE.
  Not "X is larger than Y", not "higher than", not "lower than", for any pair
  the block does not already compare. A previous answer wrote "1085m ... is
  lower than Scafell Pike at 978 meters", which is false in the direction it
  was written, and it was written because the model was left to compare the
  figures itself. Every comparison you make is one this block already made.
- DO NOT INTRODUCE A SECOND CONTRADICTION. One question, one conflict.
- Every entity, figure and unit copied exactly. Do not convert or round.
- NAME NOTHING that is in neither block.
- Never hedge, never offer alternatives. The finding is settled.
- Never mention Python, the blocks, the readers, or that you were given
  anything. Write as if you had read the document yourself.

============================= READER LINES =============================
Context and wording only. NEVER a second opinion on a value, and never a source
for a comparison.
{evidence}
...
# g13. Used when Python found nothing it could verify. Saying so explicitly is
# what stops the model manufacturing a pair -- d12's block was empty last run
# and the answer stated a conflict anyway, because the prompt still read like
# an invitation to look.
NO_CONFLICT_SYNTHESIS_PROMPT = """Answer the question below.

Python grouped every figure the readers recorded, checked each against the raw
text, and compared them for incompatibility. IT FOUND NO CONFLICT IT COULD
VERIFY for this question.

============================== HOW TO ANSWER ==============================

State what the document DOES say about the subject the question names, using
the reader lines below: the claim it makes, the figures it gives, and where
each is stated. Then say plainly that no second, incompatible value for that
subject appears.

============================== HARD RULES ==============================

- DO NOT STATE A CONTRADICTION, and do not present two figures as though they
  conflict. Both previous attempts at a question of this kind invented a
  plausible-looking pair and scored zero.
- YOU MAY NOT WRITE ANY NUMERIC COMPARISON -- no "larger than", "higher than",
  "lower than" between two figures.
- Copy every figure and name exactly. NAME NOTHING that is not below.
- Answer in full; do not decline. Never hedge about what the document does say.
...
# g14 priority 1. The general synthesis prompt with ONE addition: it names the
# field that holds the answer. Kept as a separate template so every other mode
# sends the byte-identical string g13 sent.
TALLY_SYNTHESIS_PROMPT = """Write one final answer to the question, using the two blocks below.
...
the entire document.
    ...
    THE ANSWER IS "ANSWER_VALUE". Not "records_returned", not
    "distinct_entities", not "matching_count" unless ANSWER_VALUE says so, and
    not any other number in the block. Those describe HOW the calculation was
    done -- how many lines the readers emitted, how many were set aside -- and
    none of them answers the question. A previous answer stated 36 when
    ANSWER_VALUE was 5, because the block held several numbers and named none
    of them as the answer.

    "ANSWER_SENTENCE" is that answer, already written out. OPEN WITH IT, in
    your own words, keeping the number and every entity name exactly. Then give
    the supporting detail from the text evidence.

    THIS BLOCK CANNOT BE OVERRULED. If a reader's line disagrees with it, THE
    READER IS WRONG: drop the reader's version and do not mention it.

TEXT EVIDENCE is what readers found in their own slice of the verbatim text.
They are the only source for quotations, examples and specific facts.

    Within this block, COMBINE EVERYTHING -- take the UNION, never a
    substitution. Two readers naming different things is not a conflict.

============================== HARD RULES ==============================

- STATE ANSWER_VALUE EXACTLY ONCE, as the answer. Never state a second,
  different figure for the same quantity anywhere in your reply.
- NAME NOTHING THAT IS NOT IN EITHER BLOCK. This is the most damaging error
  available to you.
- If the block lists values under "rejected_not_found_in_document" or
  "unresolved_disagreements", those were checked against the document and are
  not printed there. NEVER repeat one, and never mention that anything was
  rejected.
- When the question asks for names as well as a count, give EVERY name in
  ANSWER_SENTENCE. A name left out scores nothing.
...
- Never hedge. Commit.
- If the question is listed below as asking for several separate things, ANSWER
  EVERY ONE OF THEM, in the order given. Each is graded on its own.
...
def answer_question(doc: dict, question: str, category: str | None) -> dict:
    ...
    if not kept:
        ...
        # BUT ONLY IF THEY ACTUALLY READ. This fallback assumes every reader saw
        # its slice and found nothing, and turns that unanimous silence into a
        # finding. When the readers never ran at all, the same code asserts "No
        # part of the document states this directly. Every section was read" --
        # about a document nothing has read. A whole practice run went to disk
        # that way on a 401: eighteen questions each carrying a confident false
        # statement about the text, which under this scoring is not a missed
        # point but a forbidden hit at -0.25 apiece.
        #
        # An unanswered question and a wrongly-answered one both score zero, so
        # there is nothing to lose by raising -- and the runner records the
        # error, leaves the answer empty, and a re-run retries exactly these.
        if len(missing) == len(jobs):
            raise RuntimeError(
                f"every one of the {len(jobs)} readers failed; nothing was read, "
                f"so no claim can be made about the document"
                + (f" -- {type(failed[0][1]).__name__}: {str(failed[0][1])[:160]}"
                   if failed else ""))
    ...
    if mode == "ledger":
        ...
        # g13 step 0. The claims the readers volunteered are not enough: both
        # contradiction questions the last run lost had every needed FIGURE
        # present and verified, and no claim to set against it. Read the claims
        # out of the document, exactly as find_label_unit_outliers reads units
        # out of it. Costs no model call.
        names = sorted({(r.get("entity") or "").strip() for r in verified},
                       key=len, reverse=True)
        group_claims: list[dict] = []
        if g13_on("textclaims"):
            from_text = find_text_claims(against, names)
            group_claims = find_group_rank_claims(against, names)
            ranked = sum(1 for c in from_text if c.get("claimed_rank"))
            _log(f"  ~ ledger: {len(from_text)} claim(s) read from the document "
                 f"itself ({ranked} of them rank claims), "
                 f"{len(group_claims)} group-rank claim(s); readers volunteered "
                 f"{len(claims)}")
            claims = claims + from_text

        candidates = (find_value_conflicts(verified)
                      + find_unit_outliers(verified)
                      + check_claims(claims, verified))
        if g13_on("rank"):
            # A rank claim is refuted by an ORDERING, not by a second value, so
            # nothing above can see it. Ranked first: it is the most specific
            # thing in the pool when it fires at all.
            rank_conflicts = find_rank_conflicts(claims, group_claims,
                                                 verified, against)
            if rank_conflicts:
                _log(f"  ~ ledger: {len(rank_conflicts)} rank claim(s) "
                     f"contradicted by the document's own ordering")
            candidates = rank_conflicts + candidates
        if g10_on("labelunit"):
        ...
        one_pair = select_one_conflict(candidates, question) if g13_on("onepair") else None
        if one_pair:
            # No adjudication call. The pair was selected, attribute-matched and
            # arithmetically checked in Python; sending it to a model to be
            # judged is what lost d12, whose single correct candidate the
            # adjudicator rejected and whose computed block then went out empty.
            _log(f"  ~ ledger: {len(candidates)} candidate(s); ONE selected in "
                 f"Python (route score {route_score(one_pair, question)}, "
                 f"{one_pair['kind']}"
                 + (f"/{one_pair.get('test')}" if one_pair.get("test") else "")
                 + "); adjudication skipped")
            _log(f"  ~ {one_pair['comparison_sentence'][:150]}")
            computed_block = json.dumps(one_pair, indent=2, ensure_ascii=False)
        elif g13_on("onepair"):
            # Deliberately NOT a fallback to the pile. A pile is what the model
            # pairs freely, and every wrong answer in the last run came out of
            # one. Nothing verifiable means say so.
            _log(f"  ~ ledger: {len(candidates)} candidate(s), none survived "
                 f"validation; answering that no conflict was verified")
            computed_block = json.dumps(
                {"verified_conflict": None,
                 "note": "No pair of statements about one quantity was found to "
                         "be both comparable and incompatible."},
                indent=2, ensure_ascii=False)
        else:
            conflicts = adjudicate_conflicts(candidates, question)
            computed_block = json.dumps(
                {"verified_conflicts": conflicts} if conflicts else
                {"verified_conflicts": [],
    ...
    if mode == "tally":
        ...
        if summary and precomputed:
            ...
            # The tally prompt tells the model to state ANSWER_VALUE and no
            # other count. That is right for the ranking, and wrong for a
            # question that ALSO asks a total Python settled separately -- d01
            # wants 60 profiled parks AND how many are in Spain. Fold the
            # settled total into the sentence so the rule does not suppress it.
            if summary.get("ANSWER_SENTENCE") and precomputed.get("entries"):
                summary["ANSWER_SENTENCE"] = (
                    f"The document is built from {precomputed['entries']} "
                    f"entries in total. " + summary["ANSWER_SENTENCE"])
                summary["how_to_use"] = (
                    "ANSWER_SENTENCE contains BOTH figures the question asks "
                    "for: the total, counted from the document's own repeated "
                    "structure, and the per-category count. State both. No "
                    "other number in this block answers the question."
                ) + " " + str(summary.get("how_to_use", ""))
    ...
    try:
        if mode == "arc":
            ...
            final = call_llm(template.format(
                evidence=evidence_block, question=question,
                parts=parts_block)).strip()
        elif mode == "ledger" and g13_on("onepair"):
            # One finished comparison, or an explicit nothing. Never a pile:
            # the model no longer has a second candidate to pair with.
            template = (ONEPAIR_SYNTHESIS_PROMPT if one_pair
                        else NO_CONFLICT_SYNTHESIS_PROMPT)
            final = call_llm(template.format(
                finding=computed_block, evidence=evidence_block,
            ...
            final = call_llm(CONTRADICTION_SYNTHESIS_PROMPT.format(
                ...
                question=question, parts=parts_block)).strip()
        elif mode == "tally" and g14_on("answerfield") and "ANSWER_VALUE" in computed_block:
            final = call_llm(TALLY_SYNTHESIS_PROMPT.format(
                computed=computed_block, evidence=evidence_block,
                question=question, parts=parts_block)).strip()
        else:
...
# One cheap call before any work, on the small model. Set PREFLIGHT=0 to skip.
PREFLIGHT = os.environ.get("PREFLIGHT", "1") == "1"
CHEAP_MODEL = os.environ.get("CHEAP_MODEL", "ministral-3b-2512")


def preflight() -> None:
    """
    Prove the broker answers before spending three minutes finding out it does not.

    A run with a bad key does not fail. It completes: every reader raises, every
    question falls through to the unanimous-silence path, and a full submission
    lands on disk in 180 seconds with eighteen confident answers about a document
    nothing read. That file is indistinguishable from a real one at a glance --
    same shape, same notes, same timing block -- and on competition day there is
    one evaluation run.

    So: one call, one token, on the 3B model, before the document is even loaded.
    Failed calls cost nothing, and this one costs almost nothing when it works.
    """
    if not PREFLIGHT:
        _log("preflight     skipped (PREFLIGHT=0)")
        return
    try:
        get_client().chat.completions.create(
            model=CHEAP_MODEL, max_tokens=1,
            messages=[{"role": "user", "content": "ok"}])
    except Exception as e:                                    # noqa: BLE001
        status = getattr(e, "status_code", None)
        hint = ""
        if status == 401 or "api key" in str(e).lower():
            hint = ("\n  The broker rejected the key. Check that BROKER_API_KEY holds "
                    "the real\n  group key and not the placeholder, and that it is "
                    "exported in the SAME\n  shell as the run:\n"
                    "      export BROKER_API_KEY=...\n"
                    "      echo \"${BROKER_API_KEY:0:4}...\"   # should print its first "
                    "4 characters")
        elif status is None:
            hint = ("\n  The broker could not be reached at all. Check the network "
                    "from this host:\n      python3 -u broker_check.py")
        raise SystemExit(
            f"\nPREFLIGHT FAILED -- {type(e).__name__}: {str(e)[:200]}{hint}\n\n"
            f"  Nothing was written and nothing was spent. Fix the above and re-run;\n"
            f"  a re-run reuses whatever a previous run completed.\n")
    _log(f"preflight     broker answered on {CHEAP_MODEL}; key accepted")
