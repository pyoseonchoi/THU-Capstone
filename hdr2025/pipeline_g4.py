...
def settle_absence(question: str, body: str, tail: str,
    ...
    if present:
        ...
        for f in present:
            quote = _first_mention(f.get("matched_phrase") or f["subject"], source)
            if quote:
...
def settle_ranked_table(question: str, scope_text: str,
    ...
    out["note"] = (
        ...
        "listed without a rank is not one of the ranked entries. Use these "
        "numbers exactly."
    )
    return out
...
def answer_question(doc: dict, question: str, category: str | None) -> dict:
    ...
    if mode == "tally":
        ...
        if re.search(r"\bhow many\b|\bnumber of\b", question, re.I) and \
            ...
            if entries and re.search(r"\b(?:are|is)\s+(?:there\s+)?"
                                     ...
                                     question, re.I):
                _log(f"  ~ {entries['entries']} entries from the document's own "
                     f"repeated structure; no model called")
                return {"answer": f"The book profiles {entries['entries']} of them. "
                                  + entries["note"],
                        "computed": json.dumps(entries, indent=2, ensure_ascii=False),
                        "evidence": "(not used: settled by computation)",
                        "readers": 0, "decision": "computed"}
    ...
    if filled and _declares_no_evidence(filled) is False:
        final = f"{final}\n\n{filled}"
